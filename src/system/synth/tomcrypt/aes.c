...
const struct _cipher_descriptor rijndael_desc = { e,
                                                  6,
                                                  16,
                                                  32,
                                                  16,
...
#define f_rn(bo, bi, n, k)                                                               \
    bo[n] = ft_tab[0][byte(bi[n], 0)] ^ ft_tab[1][byte(bi[(n + 1) & 3], 1)]              \
        ^ ft_tab[2][byte(bi[(n + 2) & 3], 2)] ^ ft_tab[3][byte(bi[(n + 3) & 3], 3)]      \
        ^ *(k + n)

#define i_rn(bo, bi, n, k)                                                               \
    bo[n] = it_tab[0][byte(bi[n], 0)] ^ it_tab[1][byte(bi[(n + 3) & 3], 1)]              \
        ^ it_tab[2][byte(bi[(n + 2) & 3], 2)] ^ it_tab[3][byte(bi[(n + 1) & 3], 3)]      \
        ^ *(k + n)
...
#define f_rl(bo, bi, n, k)                                                               \
    bo[n] = fl_tab[0][byte(bi[n], 0)] ^ fl_tab[1][byte(bi[(n + 1) & 3], 1)]              \
        ^ fl_tab[2][byte(bi[(n + 2) & 3], 2)] ^ fl_tab[3][byte(bi[(n + 3) & 3], 3)]      \
        ^ *(k + n)

#define i_rl(bo, bi, n, k)                                                               \
    bo[n] = il_tab[0][byte(bi[n], 0)] ^ il_tab[1][byte(bi[(n + 3) & 3], 1)]              \
        ^ il_tab[2][byte(bi[(n + 2) & 3], 2)] ^ il_tab[3][byte(bi[(n + 1) & 3], 3)]      \
        ^ *(k + n)
...
#ifdef CLEAN_STACK
static int
_rijndael_setup(const unsigned char *key, int keylen, int numrounds, symmetric_key *skey)
#else
int rijndael_setup(
    const unsigned char *key, int keylen, int numrounds, symmetric_key *skey
)
#endif
{
    ...
    skey->rijndael.eK[0] = in_key[0];
    skey->rijndael.eK[1] = in_key[1];
    skey->rijndael.eK[2] = in_key[2];
    skey->rijndael.eK[3] = in_key[3];

    switch (k_len) {
    case 4:
        t = skey->rijndael.eK[3];
        for (i = 0; i < 10; ++i) {
            t = ls_box(ROR(t, 8)) ^ rco_tab[i];
            t ^= skey->rijndael.eK[4 * i];
            skey->rijndael.eK[4 * i + 4] = t;
            t ^= skey->rijndael.eK[4 * i + 1];
            skey->rijndael.eK[4 * i + 5] = t;
            t ^= skey->rijndael.eK[4 * i + 2];
            skey->rijndael.eK[4 * i + 6] = t;
            t ^= skey->rijndael.eK[4 * i + 3];
            skey->rijndael.eK[4 * i + 7] = t;
        }
        break;
    case 6:
        skey->rijndael.eK[4] = in_key[4];
        t = skey->rijndael.eK[5] = in_key[5];
        for (i = 0; i < 8; ++i) {
            t = ls_box(ROR(t, 8)) ^ rco_tab[i];
            t ^= skey->rijndael.eK[6 * i];
            skey->rijndael.eK[6 * i + 6] = t;
            t ^= skey->rijndael.eK[6 * i + 1];
            skey->rijndael.eK[6 * i + 7] = t;
            t ^= skey->rijndael.eK[6 * i + 2];
            skey->rijndael.eK[6 * i + 8] = t;
            t ^= skey->rijndael.eK[6 * i + 3];
            skey->rijndael.eK[6 * i + 9] = t;
            t ^= skey->rijndael.eK[6 * i + 4];
            skey->rijndael.eK[6 * i + 10] = t;
            t ^= skey->rijndael.eK[6 * i + 5];
            skey->rijndael.eK[6 * i + 11] = t;
        }
        break;
    case 8:
        skey->rijndael.eK[4] = in_key[4];
        ...
        for (i = 0; i < 7; ++i) {
            t = ls_box(ROR(t, 8)) ^ rco_tab[i];
            t ^= skey->rijndael.eK[8 * i];
            skey->rijndael.eK[8 * i + 8] = t;
            t ^= skey->rijndael.eK[8 * i + 1];
            skey->rijndael.eK[8 * i + 9] = t;
            t ^= skey->rijndael.eK[8 * i + 2];
            skey->rijndael.eK[8 * i + 10] = t;
            t ^= skey->rijndael.eK[8 * i + 3];
            skey->rijndael.eK[8 * i + 11] = t;

            t = skey->rijndael.eK[8 * i + 4] ^ ls_box(t);
            skey->rijndael.eK[8 * i + 12] = t;
            t ^= skey->rijndael.eK[8 * i + 5];
            skey->rijndael.eK[8 * i + 13] = t;
            t ^= skey->rijndael.eK[8 * i + 6];
            skey->rijndael.eK[8 * i + 14] = t;
            t ^= skey->rijndael.eK[8 * i + 7];
            skey->rijndael.eK[8 * i + 15] = t;
        }
        break;
    }
...
void rijndael_ecb_encrypt(
    const unsigned char *pt, unsigned char *ct, symmetric_key *skey
) {
    ...
    LOAD32L(b0[2], &pt[8]);
    LOAD32L(b0[3], &pt[12]);
    b0[0] ^= skey->rijndael.eK[0];
    b0[1] ^= skey->rijndael.eK[1];
    b0[2] ^= skey->rijndael.eK[2];
    b0[3] ^= skey->rijndael.eK[3];
...
void rijndael_ecb_decrypt(
    const unsigned char *ct, unsigned char *pt, symmetric_key *skey
) {
    unsigned long b0[4], b1[4], *kp;
...
int rijndael_test(void) {
    ...
    static const unsigned char pt128[16] = { 0x00, 0x11, 0x22, 0x33, 0x44, 0x55,
                                             0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb,
                                             0xcc, 0xdd, 0xee, 0xff };
    static const unsigned char key128[16] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
                                              0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
                                              0x0c, 0x0d, 0x0e, 0x0f };
    static const unsigned char ct128[16] = { 0x69, 0xc4, 0xe0, 0xd8, 0x6a, 0x7b,
                                             0x04, 0x30, 0xd8, 0xcd, 0xb7, 0x80,
                                             0x70, 0xb4, 0xc5, 0x5a };

    static const unsigned char key192[24] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
                                              0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
                                              0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11,
                                              0x12, 0x13, 0x14, 0x15, 0x16, 0x17 };
    static const unsigned char ct192[16] = { 0xdd, 0xa9, 0x7c, 0xa4, 0x86, 0x4c,
                                             0xdf, 0xe0, 0x6e, 0xaf, 0x70, 0xa0,
                                             0xec, 0x0d, 0x71, 0x91 };

    static const unsigned char key256[32] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
                                              0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d,
                                              0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14,
                                              0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b,
                                              0x1c, 0x1d, 0x1e, 0x1f };
    static const unsigned char ct256[16] = { 0x8e, 0xa2, 0xb7, 0xca, 0x51, 0x67,
                                             0x45, 0xbf, 0xea, 0xfc, 0x49, 0x90,
                                             0x4b, 0x49, 0x60, 0x89 };
    symmetric_key key;
    unsigned char tmp[2][16];

    if ((errno_var = rijndael_setup(key128, 16, 0, &key)) != CRYPT_OK) {
        errno_var = 1;
        return errno_var;
    }
    ...
    if ((errno_var = rijndael_setup(key192, 24, 0, &key)) != CRYPT_OK) {
        errno_var = 1;
        return errno_var;
    }
    ...
    if ((errno_var = rijndael_setup(key256, 32, 0, &key)) != CRYPT_OK) {
        errno_var = 1;
        return errno_var;
    }
    rijndael_ecb_encrypt(pt128, tmp[0], &key);
